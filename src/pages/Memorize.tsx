    ...
    return (
        <div className="w-full h-screen flex items-center justify-center flex-col">
            ...
            <div className="w-[400px] mt-2.5 flex justify-end items-center gap-x-[5px]">
                <button className="bg-black rounded-md py-[5px] px-2.5 text-white text-[12px] font-bold" type="button" onClick={() => setIndex((prev) => (prev > 0 ? prev - 1 : dataset.length - 1))}>
                    이전
                </button>
                <button className="bg-black rounded-md py-[5px] px-2.5 text-white text-[12px] font-bold" type="button" onClick={() => setIndex((prev) => (prev < dataset.length - 1 ? prev + 1 : 0))}>
                    다음
                </button>
            </div>
        </div>
